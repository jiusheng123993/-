...
const STORAGE_KEY = 'xinghuanhai-studyplanner-state'

function loadState(): StudyPlannerState {
  try {
...
function saveState(state: StudyPlannerState): void {
...
export function getStudyPlanById(id: string): StudyPlan | undefined {
  return loadState().plans.find(p => p.id === id)
}
