...
export class OutreachCoordinatorAdapter {
  ...
  scheduleOutreach(
    ...
    try {
      ...
      if (todayMessages.length >= config.maxDailyMessages) {
        console.warn('[OutreachCoordinatorAdapter] Daily message limit reached');
        return null;
      }
      ...
      if (!sendTime) {
        console.warn('[OutreachCoordinatorAdapter] No suitable time window found');
        return null;
      }
      ...
      console.error('[OutreachCoordinatorAdapter] scheduleOutreach error:', error);
      return null;
    }
  }
  ...
  getScheduledMessages(status?: OutreachPlan['status'], limit: number = 20): OutreachPlan[] {
    try {
      ...
      console.error('[OutreachCoordinatorAdapter] getScheduledMessages error:', error);
      return [];
    }
  }
  ...
  recordResponse(
    ...
    try {
      ...
      console.error('[OutreachCoordinatorAdapter] recordResponse error:', error);
      return false;
    }
  }
  ...
  sendMessage(messageId: string): boolean {
    try {
      ...
      console.error('[OutreachCoordinatorAdapter] sendMessage error:', error);
      return false;
    }
  }
  ...
  markDelivered(messageId: string): boolean {
    try {
      ...
      console.error('[OutreachCoordinatorAdapter] markDelivered error:', error);
      return false;
    }
  }
  ...
  getResponseStats(days: number = 30): {
    ...
    try {
      ...
      console.error('[OutreachCoordinatorAdapter] getResponseStats error:', error);
      return {
        totalSent: 0,
        responseRate: 0,
        positiveRate: 0,
