...
function saveToStorage(token: string, refreshToken: string, user: UserProfile): void {
  try {
    ...
    console.error('[AuthStore] 保存认证信息失败:', err);
  }
}
...
function clearStorage(): void {
  try {
    ...
    console.error('[AuthStore] 清除认证信息失败:', err);
  }
}
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  initialize: async () => {
    ...
    if (token && user) {
      // 验证 Token 是否有效
      const isValid = verifyToken(token, true);
  ...
  logout: async () => {
    ...
    try {
      ...
      console.error('[AuthStore] 登出失败:', err);
      clearStorage();
  ...
  updateUser: (updates: Partial<UserProfile>) => {
    ...
    try {
      ...
      console.error('[AuthStore] 更新用户信息失败:', err);
    }
