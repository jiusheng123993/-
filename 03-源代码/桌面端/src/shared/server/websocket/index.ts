...
export function authenticateWebSocket(token: string): { valid: boolean; userId?: string; role?: string } {
  if (!token) return { valid: false }

  const authContext = parseBearerToken(`Bearer ${token}`)
  if (!authContext) return { valid: false }

  return { valid: true, userId: authContext.userId, role: authContext.role }
}
