...
function loadHabitState(): HabitState {
  ...
  return { habits: [], records: [], currentStreak: {} }
}
...
function toggleHabit(state: HabitState, habitId: string, date: string): HabitState {
  const recordIndex = state.records.findIndex(r => r.habitId === habitId && r.date === date)
  const newRecords = [...state.records]
  
  if (recordIndex >= 0) {
    newRecords[recordIndex] = { ...newRecords[recordIndex], completed: !newRecords[recordIndex].completed }
  } else {
    newRecords.push({ habitId, date, completed: true })
  }
