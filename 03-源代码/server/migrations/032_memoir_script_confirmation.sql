...
-- 设计：零 status 值域变更（memoir_tasks 为迁移体系外的历史建表，status 无 CHECK 也无法确证生产约束），
...
CREATE INDEX IF NOT EXISTS idx_memoir_tasks_pending_unconfirmed
  ON memoir_tasks (created_at)
