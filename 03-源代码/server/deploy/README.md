# 星寰海后端 · 线上部署说明（2026-08-08 实测）

## 服务器现状

- 服务器：49.232.203.85（与情侣消消乐共用一台）
- 部署目录：`/opt/xinghuanhai/server`
- 进程管理：**PM2**（应用名 `xinghuanhai-server`，root 运行）
  - 启动方式：`node --import tsx src/index.ts`（直接跑 src，不用 dist）
  - 重启：`sudo pm2 restart xinghuanhai-server --update-env`
- 域名入口：`https://api.xinghuanhai.com`（nginx 配置在 `/etc/nginx/sites-enabled/xinghuanhai`，
...
- 环境变量：`/opt/xinghuanhai/server/.env`（密钥只存在这里，部署时不要覆盖）
...
3. 服务器执行：
   ...
   sudo pm2 restart xinghuanhai-server --update-env
...
- 解包层级：tgz 根目录就是 `src/`，必须 `-C /opt/xinghuanhai/server`，否则会变成 `src/src` 导致启动失败
  （`ERR_MODULE_NOT_FOUND .../src/index.ts`）。
- 不要在 `/etc/nginx/conf.d/` 再建 `api.xinghuanhai.com` 的 server 块，
  与 `sites-enabled/xinghuanhai` 冲突会被 nginx 忽略。
